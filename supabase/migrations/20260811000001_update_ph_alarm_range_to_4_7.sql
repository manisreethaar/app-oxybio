-- Migration to update pH alarm range to < 4 OR > 7
...
AS $$
BEGIN
  IF NEW.ph IS NOT NULL AND (NEW.ph < 4 OR NEW.ph > 7) THEN
...
AS $$
BEGIN
  -- pH alarm: outside 4–7
  IF NEW.ph IS NOT NULL THEN
    NEW.is_ph_alarm := (NEW.ph < 4 OR NEW.ph > 7);
  END IF;
...
RETURNS TRIGGER AS $$
BEGIN
  NEW.is_ph_alarm   := (NEW.ph IS NOT NULL AND (NEW.ph < 4 OR NEW.ph > 7));
