-- Migration to update pH alarm range to < 3.75 or > 6.5
...
AS $$
BEGIN
  IF NEW.ph IS NOT NULL AND (NEW.ph < 3.75 OR NEW.ph > 6.5) THEN
...
AS $$
BEGIN
  -- pH alarm: outside 3.75–6.5
  IF NEW.ph IS NOT NULL THEN
    NEW.is_ph_alarm := (NEW.ph < 3.75 OR NEW.ph > 6.5);
  END IF;
...
RETURNS TRIGGER AS $$
BEGIN
  NEW.is_ph_alarm   := (NEW.ph IS NOT NULL AND (NEW.ph < 3.75 OR NEW.ph > 6.5));
