-- ============================================================
-- OxyOS Batch Fermentation — Missing Columns Migration
-- Run this ONCE in Supabase SQL Editor
-- Adds all columns used by the fermentation API that may be
-- missing if the DB was not kept in sync with code updates.
-- ============================================================
...
ALTER TABLE batch_fermentation_readings
  ...
  ADD COLUMN IF NOT EXISTS incubator_equipment_id   uuid REFERENCES equipment(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS is_ph_alarm              boolean DEFAULT false,
  ADD COLUMN IF NOT EXISTS is_temp_alarm            boolean DEFAULT false,
  ADD COLUMN IF NOT EXISTS supervised_by            uuid REFERENCES employees(id) ON DELETE SET NULL,
  ...
  ADD COLUMN IF NOT EXISTS sample_incubation_id     uuid REFERENCES sample_incubation_records(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS plating_status           text CHECK (plating_status IN ('not_done','pending','done_incubating','completed','na')),
...
ALTER TABLE batch_flask_endpoints
  ...
  ADD COLUMN IF NOT EXISTS declared_by            uuid REFERENCES employees(id) ON DELETE SET NULL,
...
-- 4. Re-install the alarm auto-set trigger
...
-- 5. Re-install the batch alarm sync trigger
