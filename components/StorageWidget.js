    ...
    <div className="surface p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={fetchStats} className="p-1.5 hover:bg-slate-100 rounded-lg transition-colors" title="Refresh">
          ...
          <div className="space-y-1.5">
            {stor.buckets.map(b => (
              <div key={b.name} className="flex items-center justify-between text-xs px-3 py-2 bg-slate-50 rounded-lg border border-slate-100">
                ...
                <span className="text-slate-400 font-bold">{b.fileCount} files · {(b.totalBytes / 1024 / 1024).toFixed(2)} MB</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
          {tables
            ...
            .map(t => (
              <div key={t.table} className="flex items-center justify-between px-3 py-2 bg-slate-50 rounded-lg border border-slate-100 text-xs">
                <span className="font-semibold text-slate-600 truncate mr-2">{FRIENDLY_NAMES[t.table] || t.table}</span>
                <span className="font-black text-slate-900 shrink-0">{t.count?.toLocaleString() ?? '—'}</span>
              </div>
            ))}
        </div>
      </div>
      ...
      {showArchivePanel && (
        <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-4">
          <div className="flex items-center gap-2">
            ...
            <h4 className="text-sm font-bold text-slate-900">Data Archival & Cleanup</h4>
          </div>
          ...
          <div className="flex items-center gap-3">
            <label className="text-xs font-bold text-slate-700">Archive records older than:</label>
            <select
              value={archiveDays}
              onChange={e => setArchiveDays(Number(e.target.value))}
              className="px-3 py-1.5 border border-slate-200 rounded-lg text-xs font-bold bg-white text-slate-800 focus:ring-2 focus:ring-slate-300 outline-none"
            >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
            {[
              { type: 'attendance', label: 'Attendance Logs', icon: '📋', color: 'bg-slate-600 hover:bg-slate-700' },
