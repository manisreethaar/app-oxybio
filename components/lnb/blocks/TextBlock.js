...
function RichToolbar({ execCommand }) {
  ...
  return (
    <div className="flex items-center gap-1 px-2 py-1.5 bg-slate-50 border-b border-slate-100 flex-wrap">
      {tools.map((t, i) =>
        t.sep ? (
          <div key={i} className="w-px h-4 bg-slate-300 mx-1 shrink-0" />
        ) : (
          <button
            key={i}
            type="button"
            ...
            className="p-1.5 rounded text-slate-500 hover:bg-slate-200 hover:text-slate-800 transition-colors select-none"
          >
            {t.icon}
          </button>
        )
  ...
  return (
    <div className="w-full">
      <div className="flex items-center gap-2 mb-2 px-1">
        ...
        <span className="text-xs font-bold text-slate-500 uppercase">Text Block</span>
      </div>
      <div className="border border-slate-200 rounded-xl overflow-hidden bg-white shadow-sm">
        ...
        <div className="p-1">
          <div
            ...
            onInput={handleInput}
            onBlur={handleInput}
            className={`w-full p-4 outline-none min-h-[6rem] text-sm text-slate-700 prose prose-sm max-w-none ${!canEdit ? 'bg-transparent' : 'bg-white'}`}
            dangerouslySetInnerHTML={!canEdit ? { __html: block.content || '<span class="text-slate-400 italic text-sm">Empty text block.</span>' } : undefined}
          />
        </div>
      </div>
    </div>
