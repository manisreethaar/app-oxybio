...
const STATUS_META = {
  ...
  active:    { label: 'Active',    color: 'bg-violet-50 text-violet-700 border-violet-200' },
    ...
    <div className="max-w-7xl mx-auto space-y-8 pb-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <Link
          ...
          className="inline-flex items-center px-5 py-3 bg-violet-700 hover:bg-violet-800 text-white font-black rounded-2xl shadow-lg shadow-violet-500/20 transition-all text-sm uppercase tracking-widest"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          { label: 'Active', value: counts.active, icon: Activity, color: 'text-violet-600 bg-violet-50' },
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1 max-w-sm">
          ...
          <input
            className="w-full pl-9 pr-4 py-2.5 rounded-xl border border-slate-200 text-sm font-medium text-slate-800 bg-white focus:outline-none focus:ring-2 focus:ring-violet-500 placeholder:text-slate-400"
        ...
        <div className="relative">
          ...
          <select
            className="pl-9 pr-4 py-2.5 rounded-xl border border-slate-200 text-xs font-bold text-slate-600 bg-white focus:outline-none focus:ring-2 focus:ring-violet-500 appearance-none"
            value={sort}
      ...
      {filtered.length === 0 ? (
        <div className="glass-card rounded-2xl p-16 text-center">
          ...
          <Link href="/growth-studies/new" className="mt-4 inline-block text-violet-600 font-bold text-sm hover:underline">
            ...
            return (
              <div key={study.id} className="glass-card rounded-2xl overflow-hidden hover:shadow-lg transition-all group">
                <Link href={`/growth-studies/${study.id}`} className="p-6 block">
                  ...
                  <h3 className="font-black text-slate-800 text-base group-hover:text-violet-700 transition-colors mb-1">{study.name}</h3>
                  ...
                  {total > 0 && (
                    <div>
                      ...
                      <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
                        <div className="h-full bg-violet-500 rounded-full transition-all" style={{ width: `${total ? (done / total) * 100 : 0}%` }} />
                      </div>
                    </div>
                  )}
                </Link>
