    ...
    <div className="max-w-4xl mx-auto pb-16">
      ...
      <div className="mb-10 border-b-2 border-slate-800 pb-6">
        <div className="flex items-center gap-3 mb-2">
          <FlaskConical className="w-7 h-7 text-slate-600 print:hidden" />
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
            {[
              ...
              <div key={k.label} className="bg-slate-50 border border-slate-100 rounded-2xl p-4 text-center">
                <p className="text-xl font-black text-slate-700">{k.value}</p>
                <p className="text-xs font-black text-slate-400 uppercase tracking-wider mt-1">{k.label}</p>
              </div>
            ))}
          </div>
              ...
              <tbody>
                {measurements.map((m, i) => (
                  <tr key={m.id} className={i % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'}>
                    <td className="border border-slate-100 px-3 py-1.5 font-black text-slate-700">T+{m.actual_hour}h</td>
            ...
            <tbody>
              {plate_observations.map((obs, i) => (
                <tr key={obs.id} className={i % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'}>
                  <td className="border border-slate-100 px-3 py-1.5 font-black text-slate-700">T+{obs.time_point_hours}h</td>
                  ...
                  <td className="border border-slate-100 px-3 py-1.5">
                    <span className={`px-1.5 py-0.5 rounded text-xs font-black ${obs.result === 'sterile' ? 'bg-emerald-100 text-emerald-700' : obs.result === 'contaminated' ? 'bg-red-100 text-red-700' : 'bg-slate-100 text-slate-700'}`}>
                      {obs.result ?? 'pending'}
                    </span>
                  </td>
      ...
      <p className="text-center text-xs text-slate-300 print:block">Generated by OxyOS Research Module · {new Date().toISOString()}</p>
    </div>
  );
}
