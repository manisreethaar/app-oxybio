              ...
              {plate_observations.map((obs, i) => (
                <tr key={obs.id} className={i % 2 === 0 ? 'bg-white' : 'bg-slate-50/50'}>
                  ...
                  <td className="border border-slate-100 px-3 py-1.5">
                    <span className={`px-1.5 py-0.5 rounded text-[9px] font-black ${obs.result === 'sterile' ? 'bg-emerald-100 text-emerald-700' : obs.result === 'contaminated' ? 'bg-red-100 text-red-700' : 'bg-slate-100 text-slate-700'}`}>
                      {obs.result ?? 'pending'}
                    </span>
                  </td>
