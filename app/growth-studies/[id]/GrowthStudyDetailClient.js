...
export default function GrowthStudyDetailClient({ initialData, initialPendingIds = [] }) {
  // `id` is no longer fetched from useParams since SSR does the routing, but we can keep it for UI display or fetch calls.
  const { id } = useParams();
  ...
  const fetchStudy = useCallback(() => {
    // Kept for refresh actions/buttons
    withTimeout(fetch(`/api/growth-studies/${id}`), 45000, 'Study detail load timed out')
      .then(r => r.json())
      .then(d => {
        if (d.study) {
          setData(d);
