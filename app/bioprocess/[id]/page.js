                    ...
                    {localFactors.map((f, fi) => {
                      ...
                      return (
                        <td key={f.code} className="px-2 py-2 text-center">
                          <span className={`inline-flex items-center justify-center w-6 h-6 rounded text-[11px] font-bold ${isHigh ? 'bg-green-100 text-green-700' : isLow ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-600'}`}>
                            {level === 1 ? '+' : level === -1 ? '−' : '0'}
                          </span>
                          <div className="text-[10px] text-gray-400 mt-0.5">{actual !== '' && actual !== undefined ? actual : ''}</div>
                        </td>
                      );
                    })}
                    {nFactors === 0 && <td></td>}
