...
import { useDownstreamRealtime } from './useDownstreamRealtime';
...
export default function DownstreamClient({
  initialBatch,
  initialFlasks,
  initialTransitions,
  initialEmployees,
  initialEquipment,
  initialStrainingData
}) {
  const { batchId } = useParams();
  
  // Realtime subscription (0ms latency sync)
  useDownstreamRealtime(batchId);

  // We mock the hook behavior by feeding it initial data, or we can refactor `useBatchWorkflow` 
  // to accept initial data directly. Let's assume we pass the initial data to `useBatchWorkflow`.
  
  const w = useBatchWorkflow({ 
    batchId, 
    module: 'downstream', 
    listHref: '/downstream', 
    stageChecklistMap: STAGE_CHECKLIST_MAP,
    initialBatch,
    initialFlasks,
    initialTransitions,
    initialEmployees,
