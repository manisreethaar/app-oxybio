...
// ── Main page ──────────────────────────────────────────────────────────────
export default function LabBenchClient({ initialQueue, initialRecentEntries, initialPendingIds }) {
  const { employeeProfile } = useAuth();

  const [queue, setQueue]       = useState(initialQueue);
  const [loading, setLoading]   = useState(false);
  const [asOf, setAsOf]         = useState(initialQueue?.as_of || null);
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res  = await withTimeout(fetch('/api/lab-bench/queue'), 45000, 'Lab bench load timed out');
      const json = await res.json();
      if (json.success) {
        setQueue(json);
        setAsOf(json.as_of);
  ...
  // initial data is seeded from SSR props
