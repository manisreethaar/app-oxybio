...
// ── Main Page ──────────────────────────────────────────────────────────────
export default function GridClient({ initialSources }) {
  const searchParams = useSearchParams();
  const { employeeProfile } = useAuth();

  const [sources, setSources]         = useState(initialSources || { batches: [], growth_studies: [] });
...
function GridRow({ row, idx, isBatch, odWavelength, updateRow, toggleSkip }) {
  ...
  return (
    <>
      <tr className={clsx('transition-colors', dim ? 'bg-slate-50' : 'hover:bg-slate-50')}>
        {/* Label — editable hour for ad-hoc rows, static for scheduled rows */}
        <td className={clsx(
          'px-3 py-2 sticky left-0 z-10 transition-colors',
          dim ? 'bg-slate-50' : 'bg-white group-hover:bg-slate-50'
        )}>
          {row.is_adhoc ? (
