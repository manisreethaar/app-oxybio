...
export default function FermentationPanel({ batch, flasks, employees, employeeProfile, role, canDo, supabase, onDataSaved, onAdvanceStage, actionLoading }) {
  ...
  const fetchData = useCallback(async () => {
    const [rRes, iRes, epRes] = await Promise.all([
      ...
      supabase.from('batch_stage_inoculation').select('t_zero_time, inoculum_vol_ml, total_volume_ml: inoculation_temp_c').eq('batch_id', batch.id).single(),
      supabase.from('batch_fermentation_endpoint').select('*').eq('batch_id', batch.id).single(),
    ]);
    if (rRes.data)  setReadings(rRes.data);
    if (iRes.data)  setInocu(iRes.data);
    if (epRes.data) setEndpoint(epRes.data);
    if (flasks.length && !activeFlask) setActiveFlask(flasks[0]?.id);
  }, [batch.id, supabase, flasks, activeFlask]);

  useEffect(() => { fetchData(); }, [fetchData]);

  // Elapsed hours from T=0
  const tZero     = inocu?.t_zero_time ? new Date(inocu.t_zero_time) : null;
  const elapsedHr = tZero ? ((new Date() - tZero) / 3600000) : null;
  const minGatePassed = elapsedHr != null && elapsedHr >= 8;
  const maxExceeded   = elapsedHr != null && elapsedHr > 20;

  const latestAlarm = readings.some(r => r.is_ph_alarm || r.is_temp_alarm);

  const handleLogReading = async (e) => {
    e.preventDefault();
    if (!pH || saving) return;
    if (isIntern && !supervisedBy) { toast.warn('Select a supervisor before submitting.'); return; }
    const flask = flasks.find(f => f.id === activeFlask);
    const elapsed = tZero
      ? (new Date(isRetro && loggedAt ? loggedAt : new Date()) - tZero) / 3600000
      : null;
    setSaving(true);
    try {
      const res = await fetch(`/api/batches/${batch.id}/fermentation`, {
        ...
        body: JSON.stringify({
          type: 'reading',
          batch_id: batch.id, flask_id: activeFlask, flask_label: flask?.flask_label,
          ph: parseFloat(pH), incubator_temp_c: temp ? parseFloat(temp) : null,
  ...
  const executeEndpoint = async () => {
    ...
    try {
      // Insert endpoint
      const epData = {
        batch_id: batch.id, declared_by: employeeProfile?.id,
        total_hours: elapsedHr ? parseFloat(elapsedHr.toFixed(2)) : null,
        final_ph: finalPh, ph_out_of_range: phOOR,
      ...
      const dispRows = flasks.map(f => ({
        endpoint_id: epRow.id, flask_id: f.id, flask_label: f.flask_label,
        disposition: flaskDisp[f.id] || 'PROCEED',
        rejection_reason: flaskDisp[f.id] === 'REJECT THIS FLASK' ? (flaskDisp[`${f.id}_reason`] || '') : null,
      }));
      await supabase.from('batch_flask_endpoints').upsert(dispRows);
      // Update rejected flasks
      const rejected = flasks.filter(f => flaskDisp[f.id] === 'REJECT THIS FLASK');
      if (rejected.length) await supabase.from('batch_flasks').update({ status: 'rejected' }).in('id', rejected.map(f => f.id));
      toast.success('Fermentation endpoint declared.');
      ...
      <div className="surface p-5">
        <div className="flex items-center justify-between mb-1">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-base font-bold text-gray-900">Fermentation</h2>
            {tZero && <span className="px-2 py-0.5 bg-navy/5 border border-navy/20 rounded text-[10px] font-black text-navy">T+{elapsedHr?.toFixed(1)}hr</span>}
          </div>
          {!endpoint && (
            <button onClick={() => setShowEndpoint(s => !s)} className={`px-3 py-1.5 text-xs font-bold rounded-lg border transition-all ${showEndpoint ? 'bg-navy text-white border-navy' : 'bg-white text-gray-600 border-gray-200 hover:border-navy'}`}>
              {showEndpoint ? 'Hide Endpoint' : 'Declare Endpoint'}
            </button>
          )}
        </div>
        ...
        {latestAlarm && <div className="flex items-start gap-2 mt-2 p-3 bg-red-50 border border-red-200 rounded-lg"><AlertTriangle className="w-4 h-4 text-red-600 shrink-0 mt-0.5"/><span className="text-xs font-bold text-red-800">⚠ Active alarm — one or more readings outside acceptable range. Check immediately.</span></div>}
        {maxExceeded && !endpoint && <div className="flex items-start gap-2 mt-2 p-3 bg-amber-50 border border-amber-200 rounded-lg"><Clock className="w-4 h-4 text-amber-600 shrink-0"/><span className="text-xs font-bold text-amber-800">Maximum fermentation time exceeded (20hr). Evaluate for rejection per SOP-004.</span></div>}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        {!endpoint && (
          <div className="surface overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-100 bg-gray-50/50 flex items-center justify-between">
              <h3 className="text-sm font-bold text-gray-900">Log Reading</h3>
              <span className="text-[10px] text-gray-400 font-bold uppercase">Every 2hr per SOP-004</span>
            </div>
            {/* Flask tabs */}
            <div className="flex border-b border-gray-100 bg-gray-50/30">
              {flasks.filter(f=>f.status!=='rejected').map((f,i) => (
                <button key={f.id} onClick={() => setActiveFlask(f.id)} className={`flex-1 py-2.5 text-[11px] font-black uppercase transition-all border-b-2 ${activeFlask===f.id ? 'border-navy text-navy bg-white' : 'border-transparent text-gray-400 hover:text-gray-600'}`}>{f.flask_label}</button>
              ))}
            </div>
            <form onSubmit={handleLogReading} className="p-5 space-y-3">
              {/* pH — big, prominent (CCP) */}
              <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[10px] font-black uppercase tracking-wider text-gray-400 mb-1">Incubator Temp <span className="text-red-500">★ CCP</span></label>
                  <input type="number" step="0.1" value={temp} onChange={e=>setTemp(e.target.value)} placeholder="37.0 °C" className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm font-semibold outline-none focus:border-navy"/>
                </div>
                <div>
        ...
        <div className="surface overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100 bg-gray-50/50">
            <h3 className="text-sm font-bold text-gray-900">pH Trend</h3>
          </div>
          <div className="p-4">
          ...
          <div className="overflow-x-auto border-t border-gray-100">
            <table className="min-w-full divide-y divide-gray-100">
              <thead><tr className="bg-gray-50/50">
                ...
                <th className="px-4 py-2 text-left text-[9px] font-bold text-gray-400 uppercase">Foam</th>
              </tr></thead>
              <tbody className="divide-y divide-gray-50">
                {[...readings].reverse().map(r => (
                  <tr key={r.id} className={r.is_ph_alarm ? 'bg-red-50' : r.is_temp_alarm ? 'bg-amber-50' : 'hover:bg-gray-50/30'}>
                    ...
                    <td className="px-4 py-2 text-xs text-gray-500">{r.foam_level}</td>
                  </tr>
                ))}
                {readings.length===0 && <tr><td colSpan={5} className="px-4 py-6 text-center text-xs text-gray-400">No readings yet.</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
      ...
      {!endpoint && showEndpoint && (
        <div className="surface overflow-hidden border-2 border-navy/20">
          <div className="px-5 py-4 border-b border-gray-100 bg-navy/5 flex items-center gap-2">
            ...
            <h3 className="text-sm font-bold text-gray-900">Declare Fermentation Endpoint</h3>
            {!minGatePassed && <span className="ml-auto px-2 py-1 bg-red-100 text-red-700 text-[10px] font-black rounded-lg">🔒 Min 8hr not reached ({elapsedHr?.toFixed(1)}hr elapsed)</span>}
            {minGatePassed && readings.length === 0 && <span className="ml-auto px-2 py-1 bg-red-100 text-red-700 text-[10px] font-black rounded-lg">🔒 Must log at least one reading first</span>}
          </div>
          {minGatePassed && readings.length > 0 ? (
            <form onSubmit={handleEndpoint} className="p-5 space-y-5">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] font-black uppercase tracking-wider text-gray-400 mb-1">Final pH <span className="text-gray-400">(target 4.2–4.5)</span></label>
                  ...
                  {epPh && (parseFloat(epPh)<4.2||parseFloat(epPh)>4.5) && <p className="text-[10px] text-red-600 font-bold mt-1">⚠ Outside target — CEO confirmation required to advance</p>}
                </div>
                <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div><label className="block text-[10px] font-bold uppercase text-gray-400 mb-1">Colour Description</label>
                  <input value={colourDesc} onChange={e=>setColourDesc(e.target.value)} placeholder="e.g. Reddish-purple, consistent" className="w-full px-3 py-2 border border-gray-200 rounded-lg text-xs font-semibold outline-none"/>
                </div>
              ...
              {/* Per-flask disposition */}
              <div>
                <label className="block text-[10px] font-black uppercase tracking-wider text-gray-400 mb-2">Per-Flask Disposition</label>
                <div className="space-y-2">
                  {flasks.map(f => (
                    <div key={f.id} className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl">
                      <span className="text-sm font-black text-navy w-6">{f.flask_label}</span>
                      <div className="flex gap-2 flex-1">
                        {['PROCEED','REJECT THIS FLASK'].map(d=>(
                          <button type="button" key={d} onClick={()=>setFlaskDisp(p=>({...p,[f.id]:d}))}
                            className={`flex-1 py-1.5 text-[10px] font-black rounded-lg border transition-all ${flaskDisp[f.id]===d?(d==='PROCEED'?'bg-navy text-white border-navy':'bg-red-600 text-white border-red-600'):'bg-white text-gray-500 border-gray-200'}`}>
                            {d}
                          </button>
                        ))}
                      </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <button type="submit" disabled={savingEp} className="py-3 bg-navy hover:bg-navy-hover text-white font-bold rounded-xl text-xs uppercase tracking-wider shadow-sm disabled:opacity-50">
                  {savingEp ? 'Saving...' : 'Save Endpoint'}
                </button>
                <button type="button" disabled={savingEp||actionLoading||!endpoint} onClick={() => onAdvanceStage('straining')} className="py-3 bg-emerald-600 hover:bg-emerald-700 text-white font-bold rounded-xl text-xs uppercase tracking-wider shadow-sm disabled:opacity-40">
                  Advance → Straining
                </button>
              </div>
            </form>
            ...
            <div className="p-8 text-center text-gray-400">
              ...
              <p className="text-sm font-medium">{!minGatePassed ? `${(8-elapsedHr).toFixed(1)}hr remaining before endpoint can be declared` : 'Log at least one reading first'}</p>
            </div>
          )}
        </div>
      )}
