...
export default function SOPLibraryPage() {
  ...
  const handleUpload = async (e) => {
    e.preventDefault(); if (!uploadForm.file) return alert("Select file.");
    setUploading(true);
    try {
      const formData = new FormData(); formData.append('file', uploadForm.file);
      const uploadRes = await fetch('/api/upload', { method: 'POST', body: formData }); const uploadData = await uploadRes.json();
      const { error } = await supabase.from('sop_library').insert({ sop_id: `SOP-${Date.now().toString(36).toUpperCase().slice(-4)}`, title: uploadForm.title, category: uploadForm.category, version: uploadForm.version, document_url: uploadData.url, is_active: true });
      if (!error) { setShowUploadModal(false); setUploadForm({ title: '', category: 'QC', version: '1.0', file: null }); fetchSOPs(); }
  ...
  return (
    <div className="page-container">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {sops.map(sop => (
          <div key={sop.id} className={`surface p-5 flex flex-col hover:border-gray-300 transition-colors ${!sop.is_acknowledged ? 'border-amber-200 bg-amber-50/10' : ''}`}>
            ...
            <div className="flex justify-between items-center mt-auto pt-3 border-t border-gray-100">
              ...
              <a href={sop.document_url || '#'} target="_blank" rel="noopener noreferrer" className="p-1 text-gray-400 hover:text-navy hover:bg-gray-50 rounded-md border border-transparent hover:border-gray-200"><ExternalLink className="w-4 h-4" /></a>
            </div>
            {!sop.is_acknowledged && (
      ...
      {showAckModal && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex justify-center items-center z-50 p-4">
          <div className="bg-white rounded-xl max-w-sm w-full p-6 relative shadow-xl border border-gray-100 flex flex-col gap-4">
        ...
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex justify-center items-center z-50 p-4">
          <div className="bg-white rounded-xl max-w-sm w-full p-6 relative shadow-xl">
            ...
            <form onSubmit={handleUpload} className="space-y-3">
              <div><label className="block text-[10px] font-bold text-gray-400 mb-1">Title</label><input required type="text" value={uploadForm.title} onChange={e => setUploadForm({...uploadForm, title: e.target.value})} className="w-full border border-gray-200 rounded-lg p-2 outline-none font-semibold text-sm" /></div>
              <div className="grid grid-cols-2 gap-3">
                <div><label className="block text-[10px] font-bold text-gray-400 mb-1">Category</label><select value={uploadForm.category} onChange={e => setUploadForm({...uploadForm, category: e.target.value})} className="w-full border border-gray-200 rounded-lg p-2 outline-none bg-white text-sm font-semibold"><option value="Fermentation">Fermentation</option><option value="QC">QC</option><option value="Sanitation">Sanitation</option><option value="Safety">Safety</option></select></div>
                <div><label className="block text-[10px] font-bold text-gray-400 mb-1">Version</label><input required type="text" value={uploadForm.version} onChange={e => setUploadForm({...uploadForm, version: e.target.value})} className="w-full border border-gray-200 rounded-lg p-2 outline-none text-sm font-semibold" /></div>
              </div>
              <div><label className="block text-[10px] font-bold text-gray-400 mb-1">File (PDF)</label><input required type="file" accept=".pdf" onChange={e => setUploadForm({...uploadForm, file: e.target.files[0]})} className="w-full border border-gray-200 rounded-lg p-2 bg-gray-50 text-xs" /></div>
              <button disabled={uploading} type="submit" className="w-full bg-navy hover:bg-navy-hover text-white font-bold py-2.5 rounded-lg transition-colors text-xs uppercase tracking-wider">{uploading ? 'Uploading...' : 'Publish'}</button>
            </form>
          </div>
        </div>
      )}
