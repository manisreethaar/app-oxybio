...
const BORDERS = {
  task: 'border-blue-100 bg-blue-50/30',
    ...
    return (
      <div className="surface overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-100 bg-gray-50/50">
          <h2 className="text-base font-bold text-gray-900 tracking-tight">My Pending Actions</h2>
        </div>
          ...
          <div className="divide-y divide-gray-50">
            {actions.map((action, idx) => (
              <Link 
                ...
                href={action.link}
                className={`flex items-start gap-4 p-5 hover:bg-gray-50 transition-colors border-l-4 ${BORDERS[action.type]?.split(' ')[0]} group`}
              >
                <div className="mt-1 p-2 bg-white rounded-lg shadow-sm border border-gray-100 shrink-0">
                  {ICONS[action.type]}
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-bold text-gray-900 truncate group-hover:text-navy transition-colors">{action.title}</p>
                ...
                <ArrowRight className="w-4 h-4 text-gray-300 group-hover:text-navy transition-colors shrink-0 self-center opacity-0 group-hover:opacity-100 -ml-2 transform group-hover:translate-x-2" />
              </Link>
            ))}
          </div>
        )}
