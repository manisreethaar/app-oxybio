...
export default function PurchaseRequestsTab({ canApprove }: { canApprove: boolean }) {
  ...
  if (loading) return <div className="p-8 text-center text-slate-400 font-bold">Loading requests...</div>;

  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h2 className="text-xl font-black text-slate-800">Purchase Requisitions (PR)</h2>
      ...
      <div className="grid gap-4">
        {requests.map((r: any) => (
          <div key={r.id} className="bg-white p-5 rounded-2xl shadow-sm border border-slate-100 flex justify-between items-center">
            <div>
              <div className="flex items-center gap-3 mb-1">
                <span className="font-bold text-slate-900">{r.item_name}</span>
                <span className="text-xs font-black px-2 py-0.5 rounded bg-slate-100 text-slate-600">Qty: {r.requested_quantity} {r.unit}</span>
              ...
              <p className="text-xs font-semibold text-slate-500 mb-2">Requested by {r.requester?.full_name} • {new Date(r.created_at).toLocaleDateString()}</p>
              {r.reason && <p className="text-sm text-slate-600 bg-slate-50 p-2 rounded-lg italic">&quot;{r.reason}&quot;</p>}
            </div>
          ...
          <div className="bg-white rounded-2xl w-full max-w-md overflow-hidden">
            ...
            <form onSubmit={handleSubmit} className="p-5 space-y-4">
              ...
              <div className="pt-4 border-t border-slate-100 flex justify-end gap-3">
                <button type="button" onClick={() => setShowModal(false)} className="px-4 py-2 text-sm font-bold text-slate-500 hover:bg-slate-100 rounded-xl">Cancel</button>
