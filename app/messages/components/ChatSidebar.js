      ...
      <div className="p-4 border-b border-slate-100 flex-shrink-0">
        <div className="flex justify-between items-center mb-3">
          ...
          <button 
            ...
            className="hidden md:flex p-1.5 bg-slate-50 hover:bg-slate-100 text-navy rounded-lg transition-colors border border-slate-200"
            title="New Chat"
          >
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-accent outline-none font-medium text-slate-700"
          />
        </div>
      </div>
            ...
            return (
              <button
                ...
                className={`w-full text-left flex items-center gap-3 p-3 rounded-xl transition-colors ${
                  isActive ? 'bg-navy/5 border border-navy/10' : 'hover:bg-slate-50 border border-transparent'
                }`}
              >
                <div className="relative">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-baseline mb-0.5">
                    <h3 className={`text-sm font-bold truncate ${isActive ? 'text-navy' : 'text-slate-800'}`}>
                      {chatName}
                    </h3>
                    {unreadCount > 0 && (
