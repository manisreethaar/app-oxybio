...
export default function BatchDetailClient({ initialBatch, initialStock, initialHistory }) {
  ...
  const [inventoryStock, setInventoryStock] = useState(initialStock || []);
  ...
  const { register, handleSubmit, control, reset, watch } = useForm({
    defaultValues: {
      ingredients: [],
      notes: '',
      target_volume: initialBatch?.planned_volume_ml || '',
      target_ph: '',
      target_brix: ''
    }
  });
