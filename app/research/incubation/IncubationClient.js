...
import { useIncubationRealtime } from './useIncubationRealtime';

export default function SampleIncubationPage({ initialRecords }) {
  const { employeeProfile, role, loading: authLoading } = useAuth();
  const toast = useToast();
  useIncubationRealtime();
  ...
  const filteredSamples = useMemo(() => {
    let arr = initialRecords || [];
    if (statusFilter === 'ongoing') arr = arr.filter(r => !r.end_time);
    if (statusFilter === 'completed') arr = arr.filter(r => !!r.end_time);
    if (deferredSearch.trim()) {
      const q = deferredSearch.toLowerCase();
      arr = arr.filter(r => r.sample_name?.toLowerCase().includes(q));
    }
    return arr;
  }, [initialRecords, statusFilter, deferredSearch]);
  ...
  const grouped = useMemo(() => {
    ...
    for (const r of filteredSamples) {
  ...
  const stats = useMemo(() => {
    const now = Date.now();
    return filteredSamples.reduce((acc, s) => {
  ...
  return (
    <div className="page-container text-slate-900 space-y-6">
      ...
      {/* Reads Due Today alert panel */}
      {!loading && (
        <ReadsDuePanel samples={filteredSamples} onLogRead={openEdit} />
      )}
      ...
      {!loading && filteredSamples.length > 0 && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <ContaminationCard samples={filteredSamples} />
        </div>
      )}
      ...
      {showModal && (
        <IncubationFormModal
          ...
          onSuccess={() => { setShowModal(false); toast.success('Record saved!'); }}
        />
      )}
      <ConfirmModal
