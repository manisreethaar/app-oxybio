...
function CoaModal({ prep, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-50 backdrop-blur-sm p-4 overflow-auto">
...
// ---- Main Page ------------------------------------------------------------
export default function PrepDetailClient({ initialPrep, initialLabMedia }) {
  const { prepId } = useParams();
  const { role }   = useAuth();
  const toast      = useToast();
  const [prep, setPrep]         = useState(initialPrep);
  const [vials, setVials]       = useState(initialPrep?.cell_bank_vials || []);
  const [labMedia, setLabMedia] = useState(initialLabMedia || []);
