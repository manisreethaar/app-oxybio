...
// ---- New prep form ---------------------------------------------------------
function NewPrepForm({ strains, formulations, initialFormulationId, initialStrainId, role, employeeProfile, onSave, onCancel }) {
  const sopGate = useSopGate({ employeeId: employeeProfile?.id, category: 'QC', role, bypassRoles: ['admin', 'research_fellow', 'scientist'] });
  ...
  if (sopGate.checking || !sopGate.isTrained) {
    return (
      <div className="surface overflow-hidden">
        <SopGate checking={sopGate.checking} isTrained={sopGate.isTrained} category="QC" sopLabel="Cell Banking SOP" onClose={onCancel} />
      </div>
    );
  }
