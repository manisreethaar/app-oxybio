...
const SEVERITY_STYLE = { Minor: 'bg-slate-50 text-slate-700 border-slate-100', Major: 'bg-amber-50 text-amber-700 border-amber-100', Critical: 'bg-red-50 text-red-700 border-red-100' };
const STATUS_STYLE = { Open: 'bg-slate-100 text-slate-600', Investigating: 'bg-slate-50 text-slate-700 border-slate-100', 'CAPA Assigned': 'bg-amber-50 text-amber-700 border-amber-100', Closed: 'bg-emerald-50 text-emerald-700 border-emerald-100' };
  ...
  if (selected) return (
    <div className="max-w-4xl mx-auto space-y-6 pb-24">
        ...
        {investigation ? (
          <div className="space-y-4">
            ...
            {investigation.ishikawa_data && Object.values(investigation.ishikawa_data).some(v => v) && (
              <div>
                <h3 className="text-xs font-bold text-slate-500 uppercase tracking-wider mb-2">Ishikawa (Fishbone) Analysis</h3>
      ...
      <div className="card p-6">
        ...
        {capaActions.length === 0 ? <p className="text-xs text-slate-400 font-medium text-center py-4">No actions assigned.</p> : (
          <div className="space-y-2">
            {capaActions.map(a => (
        ...
        <div className="fixed inset-0 bg-slate-50/10 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <form onSubmit={handWhy(handleSaveInvestigation)} className="max-h-[90vh] flex flex-col overflow-y-auto bg-white rounded-xl p-6 w-full max-w-2xl shadow-xl space-y-4">
            ...
            <div className="flex bg-slate-100 p-1 rounded-lg">
              <button type="button" onClick={() => setAnalysisMethod('5why')} className={`flex-1 text-xs font-bold py-1.5 rounded-md transition-all ${analysisMethod === '5why' ? 'bg-white shadow text-navy' : 'text-slate-500 hover:text-slate-700'}`}>5-Why</button>
              <button type="button" onClick={() => setAnalysisMethod('ishikawa')} className={`flex-1 text-xs font-bold py-1.5 rounded-md transition-all ${analysisMethod === 'ishikawa' ? 'bg-white shadow text-navy' : 'text-slate-500 hover:text-slate-700'}`}>Ishikawa (Fishbone)</button>
            </div>
  ...
  const filteredDeviations = deviations
    .filter(dev => {
      ...
      const matchesSearch = !q || [
        dev.title, dev.description, dev.source, dev.status, dev.severity,
        dev.reporter?.full_name, dev.batches?.batch_id
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div><h2 className="text-xl font-bold text-slate-900 tracking-tight">CAPA Manager</h2><p className="text-sm text-slate-500 mt-1">Non-Conformance & Action items.</p></div>
      ...
      <div className="flex flex-col lg:flex-row gap-3 lg:items-center">
        <div className="relative flex-1 min-w-[220px]">
          ...
          <input value={searchTerm} onChange={e => setSearchTerm(e.target.value)} placeholder="Search NCR title, source, reporter, or batch..." className="w-full pl-9 pr-3 py-2 border border-slate-200 rounded-lg text-xs bg-white font-semibold text-slate-700 outline-none focus:ring-2 focus:ring-accent" />
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          <select value={severityFilter} onChange={e => setSeverityFilter(e.target.value)} className="px-3 py-2 border border-slate-200 rounded-lg text-xs bg-white font-bold text-slate-600 outline-none">
          ...
          {filteredDeviations.map(dev => (
            <div key={dev.id} role="button" tabIndex={0} onClick={() => loadDetail(dev)} onKeyDown={e => e.key === 'Enter' && loadDetail(dev)} className="w-full card p-4 flex items-center justify-between hover:border-slate-300 transition-colors text-left cursor-pointer">
              ...
              <div className="flex items-center gap-2 shrink-0">
                {dev.creator && <CreatorBadge initials={dev.creator.initials} fullName={dev.creator.full_name} size="sm"/>}
                {!isAdmin && dev.created_by === employeeProfile?.id && dev.status === 'Open' && (
                  <div onClick={e => e.stopPropagation()}>
                    <EditRequestButton
                      tableName="deviations" recordId={dev.id} moduleLabel="Deviations"
                      fields={[
                      ...
                      currentData={dev} hasPending={pendingIds.has(dev.id)}
