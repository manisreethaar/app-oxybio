  ...
  const fetchLeaves = async () => {
    ...
    try {
      if (['admin', 'ceo', 'cto'].includes(role)) {
        ...
        ]), 45000, 'Leave load timed out');
        setLeaves(myLeavesRes.data || []);
        setPendingLeaves(pLeavesRes.data || []);
      } else {
        const { data: myLeaves } = await withTimeout(supabase.from('leave_applications').select('*').eq('employee_id', employeeProfile.id).order('created_at', { ascending: false }).limit(500), 45000, 'Leave load timed out');
        setLeaves(myLeaves || []);
      }
