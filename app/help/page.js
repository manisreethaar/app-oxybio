        ...
        <ol key={i} className="my-2 space-y-1.5 list-none">
          {steps.map((s, j) => (
            <li key={j} className="flex gap-3 text-sm text-slate-700 leading-relaxed">
              <span className="flex-shrink-0 w-5 h-5 rounded-full bg-slate-600 text-white text-xs flex items-center justify-center font-bold">
                {j + 1}
              </span>
...
function inlineFormat(text) {
  ...
  return parts.map((part, i) => {
    ...
    if (part.startsWith('`') && part.endsWith('`')) {
      return <code key={i} className="px-1 py-0.5 bg-slate-100 text-slate-700 text-xs rounded font-mono">{part.slice(1, -1)}</code>;
    }
    return part;
  });
}
...
function Article({ article, defaultOpen = false, searchQuery = '' }) {
  ...
  return (
    <div className="border border-slate-200 rounded-xl overflow-hidden transition-all hover:border-slate-200">
      <button
        onClick={() => setOpen(o => !o)}
        className="w-full flex items-center justify-between gap-3 px-4 py-3.5 text-left bg-white hover:bg-slate-50 transition-colors group"
        aria-expanded={open}
      >
      ...
      {open && (
        <div
          ref={contentRef}
          className="px-4 pb-4 pt-1 bg-white border-t border-slate-100"
        >
...
function QuickAction({ action }) {
  ...
  return (
    <a
      href={action.href}
      className="flex items-center gap-2 px-3.5 py-2 bg-white border border-slate-200 rounded-xl text-sm font-semibold text-slate-700
                 hover:bg-slate-600 hover:border-slate-600 hover:text-white transition-all shadow-sm group"
    >
      <Icon className="w-4 h-4 text-slate-500 group-hover:text-white transition-colors flex-shrink-0" />
      {action.label}
    </a>
  );
}
...
const ROLE_BADGE_STYLES = {
  ...
  fellow: 'bg-slate-100 text-slate-700 border-slate-200',
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-5 pb-24 print:max-w-none print:pb-0">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <div>
                <h1 className="text-2xl font-black text-slate-900 tracking-tight">Help & Manual</h1>
                <p className="text-xs text-slate-400 mt-0.5">OxyOS Platform · Oxygen Bioinnovations</p>
              </div>
            </div>
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-slate-600 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 hover:border-slate-300 transition-all shadow-sm flex-shrink-0"
          >
      ...
      <div className="hidden print:block mb-6">
        <h1 className="text-2xl font-black text-slate-900">OxyOS — Help & Manual</h1>
        <p className="text-slate-500 text-sm mt-1">Oxygen Bioinnovations · Internal Use Only · {roleGroupLabel}</p>
      </div>

      {/* ── Quick Actions ── */}
      {content?.quickActions && (
        <div className="print:hidden">
          <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest mb-2.5">Quick Access</p>
      ...
      <div className="relative print:hidden">
        ...
        <input
          ...
          className="w-full pl-10 pr-10 py-3 bg-white border border-slate-200 rounded-xl text-sm text-slate-800
                     placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-500 focus:border-transparent
      ...
      <div className="space-y-4">
        ...
        {searchQuery && visibleSections.every(s => {
          ...
          <div className="text-center py-16">
            ...
            <button
              onClick={() => setSearchQuery('')}
              className="text-sm font-semibold text-slate-600 hover:text-slate-700 transition-colors"
            >
              Clear search
            </button>
          </div>
      ...
      {/* ── Footer ── */}
      <div className="mt-6 p-5 bg-gradient-to-r from-slate-50 to-slate-50 border border-slate-100 rounded-2xl print:hidden">
        <p className="text-sm font-bold text-slate-800 mb-1">Still need help?</p>
        <p className="text-sm text-slate-600">
          Use the <span className="font-semibold text-slate-700">AI Assistant</span> (chat bubble icon) for quick answers,
