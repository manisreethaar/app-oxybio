                  ...
                  {columnItems.length === 0 ? (
                    ...
                    return (
                      <div key={device.id} className="bg-white p-3 rounded-lg border border-slate-200 shadow-sm hover:shadow-md transition-all flex flex-col gap-2 group cursor-pointer" onClick={() => { setModalMode('edit'); setActiveDevice(device); resetEquip({...device}); setIsModalOpen(true); }}>
                          ...
                          {isAdmin && (
                            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-all">
                               <button 
                                 ...
                                 className="p-1 rounded bg-gray-50 text-gray-400 hover:bg-red-50 hover:text-red-600 transition-all border border-gray-200"
                               >
                                 <Trash2 className="w-3 h-3" />
                               </button>
                            </div>
                        ...
                        <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider truncate">{device.model || 'Standard'} — {device.serial_number || 'SN-UNKNOWN'}</p>
