...
import { useEquipmentRealtime } from './useEquipmentRealtime';
...
export default function EquipmentClient({ initialEquipment = [], initialSterilData = [] }) {
  ...
  useEffect(() => {
    ...
    if (saved && ['grid', 'kanban', 'table'].includes(saved)) {
      setViewMode(saved);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const usageMap = {};
    (initialSterilData || []).forEach(row => {
      if (!usageMap[row.equipment_id] && row.batches) usageMap[row.equipment_id] = row.batches;
    });
    setBatchUsageMap(usageMap);
  }, [initialEquipment, initialSterilData]);

  const fetchEquipment = async () => {
    // Relying on server component refresh via Realtime hook, but keep as no-op to prevent breaking existing calls
  };
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 pb-20">
      ...
      <div className="flex flex-col lg:flex-row gap-3 justify-between">
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto flex-1 lg:flex-initial">
      ...
      {viewMode === 'grid' && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          ...
          return (
            <div key={device.id} className="bg-white rounded-xl border px-3 py-2.5 relative group overflow-hidden transition-all hover:shadow-sm border-slate-100 hover:border-slate-200 flex flex-col h-full">
              ...
              <div className="flex justify-between items-start mb-2">
                <span className={`px-2 py-0.5 rounded text-[10px] sm:text-xs font-black uppercase tracking-widest ${device.status === 'Operational' ? 'bg-slate-100 text-slate-500' : 'bg-red-100 text-red-700'}`}>
                  {device.status}
                </span>
                {isAdmin && (
                  <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-all">
                    <button 
                      ...
                      className="p-1.5 rounded-lg bg-slate-50 text-slate-400 hover:bg-slate-50 hover:text-slate-600 transition-all border border-slate-200 shadow-sm"
                    >
                      <Settings className="w-3 h-3" />
                    </button>
                    <button 
                      onClick={() => setDeletingId(device.id)}
                      className="p-1.5 rounded-lg bg-slate-50 text-slate-400 hover:bg-red-50 hover:text-red-600 transition-all border border-slate-200 shadow-sm"
                    >
              ...
              <div className="flex items-center justify-between mb-2">
                <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider truncate">{device.model || 'Standard Unit'} — {device.serial_number || 'SN-UNKNOWN'}</p>
                {(() => {
              ...
              <div className="flex-1 flex flex-col gap-4 mt-auto pt-2 border-t border-slate-50">
      ...
      {viewMode === 'kanban' && (
        <div className="flex gap-4 overflow-x-auto pb-4 snap-x">
          {['Operational', 'Under Maintenance', 'Out of Service'].map(statusColumn => {
            const columnItems = filteredEquipment.filter(d => d.status === statusColumn);
            ...
            return (
              <div key={statusColumn} className="w-80 shrink-0 snap-start flex flex-col max-h-[calc(100vh-200px)]">
                <div className="bg-slate-200/50 rounded-t-xl p-3 border border-b-0 border-slate-200 flex flex-col gap-1.5 shrink-0">
                  <div className="flex items-center justify-between">
                    <span className="font-black text-slate-900 truncate">{statusColumn}</span>
                    <span className={`text-[10px] font-black uppercase px-2 py-0.5 rounded border ${
                      statusColumn === 'Operational' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' :
                      statusColumn === 'Under Maintenance' ? 'bg-amber-50 text-amber-700 border-amber-200' :
                    ...
                    return (
                      <div key={device.id} className="bg-white p-3 rounded-lg border border-slate-200 shadow-sm hover:shadow-md transition-all flex flex-col gap-2 group cursor-pointer" onClick={() => { setModalMode('edit'); setActiveDevice(device); resetEquip({...device}); setIsModalOpen(true); }}>
                        <div className="flex justify-between items-start">
                          <h3 className="text-xs font-black text-indigo-700 line-clamp-2">{device.name}</h3>
                          {isAdmin && (
                            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-all">
                               <button 
                                 onClick={(e) => { e.stopPropagation(); setDeletingId(device.id); }}
                                 className="p-1 rounded bg-slate-50 text-slate-400 hover:bg-red-50 hover:text-red-600 transition-all border border-slate-200"
                               >
                                 <Trash2 className="w-3 h-3" />
                               </button>
                            </div>
                        ...
                        <p className="text-[10px] font-bold text-slate-500 uppercase tracking-wider truncate">{device.model || 'Standard'} — {device.serial_number || 'SN-UNKNOWN'}</p>
                        
                        <div className="flex gap-2 mt-1">
                          {device.requires_calibration && (
                             <div className={`px-2 py-1 flex flex-1 items-center justify-between rounded border ${isCalibrationDue ? 'bg-red-50 border-red-100 text-red-700' : 'bg-slate-50 border-slate-100 text-slate-600'}`}>
                               <span className="text-[9px] font-black uppercase">Calib</span>
                               <span className="text-[9px] font-mono">{device.calibration_due_date ? new Date(device.calibration_due_date).toLocaleDateString() : 'N/A'}</span>
                             </div>
                          )}
                          <div className={`px-2 py-1 flex flex-1 items-center justify-between rounded border ${isPmDue ? 'bg-red-50 border-red-100 text-red-700' : 'bg-slate-50 border-slate-100 text-slate-600'}`}>
                             <span className="text-[9px] font-black uppercase">PM</span>
                             <span className="text-[9px] font-mono">{device.next_pm_date ? new Date(device.next_pm_date).toLocaleDateString() : 'N/A'}</span>
                          </div>
                        </div>
                        ...
                        <div className="flex gap-1 mt-1">
                           <button
                             disabled={!['admin', 'ceo', 'cto'].includes(role)}
                             onClick={(e) => { e.stopPropagation(); setActiveDevice(device); setMaintValue('status', device.status); setMaintValue('equipment_id', device.id); setMaintValue('log_type', device.requires_calibration ? 'Calibration' : 'Maintenance'); setIsMaintenanceOpen(true); }}
                             className="flex-1 py-1 bg-slate-100 text-slate-600 rounded text-[9px] font-black uppercase hover:bg-slate-200 disabled:opacity-50">Log</button>
                           <button 
                             onClick={(e) => { e.stopPropagation(); setActiveDevice(device); setTicketValue('equipment_id', device.id); setIsTicketOpen(true); }} 
                             className="flex-1 py-1 bg-red-50 text-red-600 rounded text-[9px] font-black uppercase hover:bg-red-100">Issue</button>
                        </div>
                      </div>
                    );
                  })}
          ...
          <table className="w-full text-left border-collapse min-w-[800px]">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                <th className="px-4 py-3 text-xs font-black uppercase tracking-wider text-slate-500">Asset Name & Model</th>
                <th className="px-4 py-3 text-xs font-black uppercase tracking-wider text-slate-500">Serial No.</th>
                <th className="px-4 py-3 text-xs font-black uppercase tracking-wider text-slate-500">Status</th>
                <th className="px-4 py-3 text-xs font-black uppercase tracking-wider text-slate-500">Calibration Due</th>
                <th className="px-4 py-3 text-xs font-black uppercase tracking-wider text-slate-500">PM Due</th>
                ...
                return (
                  <tr key={device.id} className="hover:bg-slate-50 transition-colors">
                    <td className="px-4 py-2">
                      <div className="text-sm font-semibold text-indigo-700">{device.name}</div>
                      <div className="text-[10px] font-bold text-slate-400 uppercase">{device.model || 'Standard Unit'}</div>
                    </td>
                    <td className="px-4 py-2 text-sm font-mono text-slate-600">{device.serial_number || 'N/A'}</td>
                    <td className="px-4 py-2">
                      <span className={`text-[10px] font-black uppercase px-2 py-0.5 rounded border ${
                         device.status === 'Operational' ? 'bg-emerald-50 text-emerald-700 border-emerald-200' :
                         device.status === 'Under Maintenance' ? 'bg-amber-50 text-amber-700 border-amber-200' :
                         'bg-red-50 text-red-700 border-red-200'
                      }`}>{device.status}</span>
                    </td>
                    <td className="px-4 py-2 text-sm font-mono">
                      {device.requires_calibration ? (
                        <span className={isCalibrationDue ? 'text-red-600 font-black' : 'text-slate-600'}>
                          {device.calibration_due_date ? new Date(device.calibration_due_date).toLocaleDateString() : 'N/A'}
                        </span>
                      ) : <span className="text-slate-300">-</span>}
                    </td>
                    <td className="px-4 py-2 text-sm font-mono">
                      <span className={isPmDue ? 'text-red-600 font-black' : 'text-slate-600'}>
                        {device.next_pm_date ? new Date(device.next_pm_date).toLocaleDateString() : 'N/A'}
                      </span>
                    </td>
                    <td className="px-4 py-2">
                      <div className="flex justify-center gap-1">
                         {isAdmin && (
                           <button onClick={() => { setModalMode('edit'); setActiveDevice(device); resetEquip({...device}); setIsModalOpen(true); }} className="px-2 py-1 bg-slate-100 hover:bg-slate-200 text-slate-700 text-xs font-bold rounded">Edit</button>
                         )}
                         <button
                           disabled={!['admin', 'ceo', 'cto'].includes(role)}
                           ...
                           className="px-2 py-1 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 text-xs font-bold rounded disabled:opacity-50">Log</button>
                         <button 
                           onClick={() => { setActiveDevice(device); setTicketValue('equipment_id', device.id); setIsTicketOpen(true); }} 
                           className="px-2 py-1 bg-red-50 hover:bg-red-100 text-red-700 text-xs font-bold rounded">Issue</button>
                         {isAdmin && (
                           <button onClick={() => setDeletingId(device.id)} className="px-2 py-1 bg-slate-50 hover:bg-red-50 text-slate-400 hover:text-red-600 text-xs font-bold rounded transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
                         )}
                      </div>
                    </td>
                  </tr>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-start sm:items-center justify-center p-0 sm:p-4 bg-slate-50 backdrop-blur-sm">
