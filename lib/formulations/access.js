...
export function isFormulationApprover(employee, email) {
  return isMasterAdmin(email) || Boolean(employee?.role && can(employee.role, 'recipes', 'approve', employee.custom_permissions));
}
...
export function canCreateFormulation(employee, email) {
  ...
  return can(employee.role, 'recipes', 'create', employee.custom_permissions)
    ? { allowed: true }
...
// Recipe code: 1–5 uppercase letters optionally followed by up to 3 digits
// e.g. R01, RKU, RKU01
