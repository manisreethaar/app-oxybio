...
export async function getLinkedEquipment(supabase, batchId) {
  const EQUIPMENT_SELECT = 'equipment(id, name, model, requires_calibration, calibration_due_date, status)';

  // batch_stage_sterilisation is the legacy pre-Seed-Train pipeline's only
  // equipment source — batches created through the live Protocol/Seed/
  // Production flow never populate it, so equipment recorded during Harvest
  // (batch_stage_harvest.equipment_id, one row per flask) must be included
  // too, or every current batch shows no equipment at all.
  const [sterilRes, harvestRes] = await Promise.all([
    supabase.from('batch_stage_sterilisation').select(EQUIPMENT_SELECT).eq('batch_id', batchId).maybeSingle(),
    supabase.from('batch_stage_harvest').select(EQUIPMENT_SELECT).eq('batch_id', batchId),
  ]);

  const items = [];
  const sterilEq = sterilRes.data?.equipment;
  if (sterilEq) items.push(...(Array.isArray(sterilEq) ? sterilEq : [sterilEq]));
  for (const row of harvestRes.data || []) {
    if (row.equipment) items.push(row.equipment);
  }

  // De-dupe — the same equipment can show up across stages or multiple flasks.
  const seen = new Set();
  return items.filter(e => e?.id && !seen.has(e.id) && seen.add(e.id));
}
