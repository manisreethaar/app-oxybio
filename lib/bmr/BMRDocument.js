      ...
      <Page size="A4" style={S.page}>
        ...
        <View style={S.table}>
          <View style={[S.tableHead, { backgroundColor: C.gray900 }]}>
            ...
            <Text style={[S.tableHCell, { flex: 1.5 }]}>User & Role</Text>
            <Text style={[S.tableHCell, { flex: 0.8 }]}>Action</Text>
            <Text style={[S.tableHCell, { flex: 1.5 }]}>Target</Text>
            <Text style={[S.tableHCell, { flex: 2.5 }]}>Reason / Justification</Text>
          </View>
            ...
            data.auditLogs.map((log, i) => (
              <View key={log.id || i} style={[S.tableRow, i % 2 === 0 && S.tableRowAlt]} wrap={false}>
                ...
                <Text style={[S.tableDCell, { flex: 1.5 }]}>
                  {log.employees?.full_name || 'System / Admin'}
                  {log.employees?.role ? ` (${log.employees.role})` : ''}
                </Text>
                <Text style={[S.tableDCell, { flex: 0.8, color: log.action === 'DELETE' ? C.red : log.action === 'INSERT' ? C.emerald : C.navy, fontFamily: 'Helvetica-Bold' }]}>{log.action}</Text>
                <Text style={[S.tableDCell, { flex: 1.5 }]}>{log.table_name}</Text>
                <Text style={[S.tableDCell, { flex: 2.5, fontStyle: 'italic', color: log.reason ? C.gray900 : C.gray500 }]}>
                  {log.reason || 'Routine system entry'}
                </Text>
              </View>
            ))
          )}
