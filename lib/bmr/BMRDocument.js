...
export function BMRDocument({ data }) {
  const {
    ...
    flaskInoculations, flaskReadings, flaskEndpoints, flaskHarvest, flaskStraining,
    flaskExtracts, flaskQCSamples, flaskQCTests, flaskReleases, flaskRejections,
    sampleIncubations, inventoryUsage, seedTrains, generatedBy, generatedAt
  } = data;
  ...
  return (
    <Document author="OxyOS" title={`BMR — ${batch.batch_id}`} subject="Batch Manufacturing Record">
      ...
      <Page size="A4" style={S.page}>
        ...
        {/* §3b Seed Train — batches originated via Protocol -> Seed 1/2/3 ->
            Production record their media/sterilisation/inoculation here
            instead of the legacy global media prep/sterilisation tables. */}
        {seedTrains?.length > 0 && (
          <View style={S.section}>
            <SectionHdr num="§3b" title="Seed Train" status={`${seedTrains.length} Stage${seedTrains.length === 1 ? '' : 's'}`} />
            <View style={S.sectionBody}>
              {seedTrains.map((st, i) => (
                <View key={st.id || i} style={S.dataGrid}>
                  <Meta label="Stage" value={(st.stage_type || '').replace('_', ' ').toUpperCase()} />
                  <Meta label="Formulation" value={st.formulations?.name ? `${st.formulations.name} v${st.formulations.version}` : 'Inherited from batch'} />
                  <Meta label="Media Volume (ml)" value={st.media_volume_ml} />
                  <Meta label="Sterilised" value={st.is_sterilised ? fmt(st.sterilised_at) : 'Not recorded'} ccp />
                  <Meta label="Inoculum Source" value={st.inoculum_source_type} />
                  <Meta label="Inoculated" value={fmt(st.inoculated_at)} ccp />
                  {st.media_recipe_notes && <View style={S.noteBox}><Text style={S.noteText}>{st.media_recipe_notes}</Text></View>}
                </View>
              ))}
            </View>
          </View>
      ...
      {flasks?.map((flask, idx) => {
        ...
        const harv = flaskHarvest?.find(x => x.flask_id === flask.id);
        ...
        return (
          <Page key={flask.id} size="A4" style={S.page}>
            ...
            {/* §C Harvest */}
            <View style={S.section} wrap={false}>
              <SectionHdr num="§C" title="Harvest" status={harv ? 'Complete' : 'Pending'} />
              <View style={S.sectionBody}>
                {harv ? (
                  <View style={S.dataGrid}>
                    <Meta label="Wet Cell Weight (g)" value={harv.wet_cell_weight_g} ccp />
                    <Meta label="Final Culture Vol (L)" value={harv.final_culture_vol_l} />
                    <Meta label="Volume Recovered (L)" value={harv.volume_recovered_l} />
                    <Meta label="Biomass Yield %" value={harv.biomass_yield_pct} ccp />
                    <Meta label="Harvest Temp (°C)" value={harv.harvest_temp_c} />
                    <Meta label="Cell Viability %" value={harv.cell_viability_pct} />
                    <Meta label="Cooling Time (min)" value={harv.cooling_time_mins} />
                    <Meta label="Hold Temp (°C)" value={harv.hold_temp_c} />
                  </View>
                ) : <Text style={S.noteText}>No harvest data recorded.</Text>}
                {harv?.notes && <View style={S.noteBox}><Text style={S.noteText}>{harv.notes}</Text></View>}
              </View>
            </View>

            {/* §D Straining */}
            <View style={S.section} wrap={false}>
              <SectionHdr num="§D" title="Straining" status={str ? 'Complete' : 'Pending'} />
              <View style={S.sectionBody}>
                {str ? (
                  <View style={S.dataGrid}>
            ...
            {/* §D Extract Addition */}
            <View style={S.section} wrap={false}>
              <SectionHdr num="§E" title="Extract Integration" status={ex ? 'Complete' : 'Pending'} />
              <View style={S.sectionBody}>
                {ex ? (
                  <View style={S.dataGrid}>
            ...
            {/* §E Quality Control */}
            <View style={S.section} wrap={false}>
              <SectionHdr num="§F" title="Trial QC Tests" status={qcT?.length > 0 && qcT.every(t => t.pass_fail !== 'Pending') ? (qcT.some(t => t.pass_fail === 'Fail') ? 'Fail' : 'Pass') : 'Pending'} />
              <View style={S.sectionBody}>
            ...
            {/* §F Disposition */}
            <View style={S.section} wrap={false}>
              <SectionHdr num="§G" title="Trial Disposition" status={rel ? 'Released' : rej ? 'Rejected' : 'Pending'} />
              <View style={S.sectionBody}>
                {rel ? (
                  <View style={S.dataGrid}>
