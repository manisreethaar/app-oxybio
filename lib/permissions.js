// ─────────────────────────────────────────────────────────────────────────────
// OxyOS RBAC - Permissions Matrix
// ─────────────────────────────────────────────────────────────────────────────
// Role hierarchy (highest to lowest):
//   ceo > cto > research_fellow > scientist > research_intern > intern
//
// DB role values must match exactly:
//   'ceo', 'cto', 'research_fellow', 'scientist', 'research_intern', 'intern'
//
// Legacy roles still supported: 'admin', 'staff'
// ─────────────────────────────────────────────────────────────────────────────

export const ROLES = {
  CEO:             'ceo',
  CTO:             'cto',
  RESEARCH_FELLOW: 'research_fellow',
  SCIENTIST:       'scientist',
  RESEARCH_INTERN: 'research_intern',
  INTERN:          'intern',
  // Legacy
  ADMIN:           'admin',
};

export const ROLE_WEIGHTS = {
  [ROLES.CEO]:             100,
  [ROLES.CTO]:             90,
  [ROLES.ADMIN]:           80,
  [ROLES.RESEARCH_FELLOW]: 70,
  [ROLES.SCIENTIST]:       60,
  [ROLES.RESEARCH_INTERN]: 45,
  [ROLES.INTERN]:          40,
};

// Shorthand role groups for reuse
const ALL_ROLES = ['ceo', 'cto', 'admin', 'research_fellow', 'scientist', 'research_intern', 'intern'];
const LEADERSHIP = ['ceo', 'cto', 'admin'];
const SENIOR = ['ceo', 'cto', 'admin', 'research_fellow'];
const SCIENTISTS = ['ceo', 'cto', 'admin', 'research_fellow', 'scientist'];

const PERMISSIONS = {

  dashboard: {
    view: ALL_ROLES,
  },

  activity: {
    view: ALL_ROLES,
    log:  ALL_ROLES,
  },

  attendance: {
    view:           ALL_ROLES,
    check_in:       ALL_ROLES,
    view_team:      SENIOR,
    export:         LEADERSHIP,
    override:       LEADERSHIP,  // geo override for CEO/CTO
  },

  tasks: {
    view:     ALL_ROLES,
    create:   ALL_ROLES,
    edit_own: ALL_ROLES,
    delete:   SENIOR,
    approve:  LEADERSHIP,
    assign:   SENIOR,
  },

  leave: {
    view:     ALL_ROLES,
    apply:    ALL_ROLES,
    approve:  LEADERSHIP,
    view_all: LEADERSHIP,
  },

  sops: {
    view:        ALL_ROLES,
    create:      SENIOR,
    edit:        SENIOR,
    delete:      LEADERSHIP,
    acknowledge: ALL_ROLES,
  },

  documents: {
    view:   ALL_ROLES,
    upload: SENIOR,
    delete: LEADERSHIP,
  },

  recipes: {
    view:    ALL_ROLES,
    create:  SCIENTISTS,
    edit:    SENIOR,
    approve: LEADERSHIP,
    delete:  LEADERSHIP,
  },

  batches: {
    view:             ALL_ROLES,
    create:           SENIOR,                                              // schedule batch: research_fellow+

    // ── Stage data entry ──────────────────────────────────────────
    // GMP supervised-entry policy:
    //   intern / research_intern → must select a supervising scientist/fellow
    //   The supervisor name is written into the stage record for audit trail.
    // ───────────────────────────────────────────────────────────────
    log_reading:      ALL_ROLES,                                           // fermentation readings — all roles (supervised)
    media_prep:       ALL_ROLES,                                           // all roles (supervised)
    sterilisation:    ALL_ROLES,                                           // all roles (supervised)
    inoculation:      ALL_ROLES,                                           // all roles (supervised)
    straining:        ALL_ROLES,                                           // all roles (supervised)
    extract_addition: ALL_ROLES,                                           // all roles (supervised)
    qc_entry:         ALL_ROLES,                                           // all roles (supervised)
    declare_endpoint: ALL_ROLES,                                           // all roles (supervised)

    // ── Disposition (terminal) ──────────────────────────────────────────
    release:          ['ceo', 'admin'],                                    // CEO ONLY — GMP hard rule
    reject:           ['ceo', 'admin'],

    // ── Admin ───────────────────────────────────────────────────────────────
    delete:           LEADERSHIP,
    export_bmr:       SENIOR,

    // Legacy (kept for existing code)
    edit:             ALL_ROLES,
    sign_off:         LEADERSHIP,
  },

  rnd_experiments: {
    view:   ALL_ROLES,
    create: SENIOR,        // consumes real inventory — same tier as batch creation
    review: LEADERSHIP,    // approve/reject a submitted experiment
  },

  compliance: {
    view:       ALL_ROLES,
    create_ncr: ALL_ROLES,
    close_capa: LEADERSHIP,
    delete:     LEADERSHIP,
  },

  equipment: {
    view:   ALL_ROLES,
    edit:   ALL_ROLES,          // Add maintenance logs (Interns +)
    create: SCIENTISTS,         // Add Machine (Scientist +)
    delete: LEADERSHIP,
  },

  inventory: {
    view:            ALL_ROLES,
    edit:            ALL_ROLES,    // Add/Deduct stock, receive stock (Interns +)
    create:          SCIENTISTS,   // Register new items, edit item catalog, edit vendors (Scientist +)
    delete:          LEADERSHIP,
    request_stock:   ALL_ROLES,
    approve_request: LEADERSHIP,
    qc_release:      SCIENTISTS,   // QC release / rejection of quarantined stock (Scientist +)
  },

  lab_notebook: {
    view:   ALL_ROLES,
    create: ALL_ROLES,          // Draft LNB (Interns +)
    finalise: SENIOR,           // Countersign LNB (Research Fellow +)
  },

  payslips: {
    view_own: ALL_ROLES,
    upload:   LEADERSHIP,
    view_all: LEADERSHIP,
  },

  notifications: {
    view:     ALL_ROLES,
    send:     SENIOR,
    send_all: LEADERSHIP,
  },

  // Research modules — Cell Bank and Incubation
  cell_bank: {
    view:             ALL_ROLES,
    create:           SENIOR,      // register strains, start preparations (research_fellow+)
    edit:             SENIOR,      // enter step data, register vials
    register_vials:   SENIOR,
    delete:           LEADERSHIP,  // delete strains/preps — admin only
  },

  incubation: {
    view:   ALL_ROLES,
    create: SENIOR,     // create incubation records (research_fellow+)
    edit:   SENIOR,
    delete: LEADERSHIP,
  },

  // Admin panel — user management
  admin: {
    view:         LEADERSHIP,
    manage_users: LEADERSHIP,
    invite:       LEADERSHIP,
    manage_roles: ['ceo', 'admin'], 
  },

  profile: {
    view: ALL_ROLES,
    edit: ALL_ROLES,
  },

  directory: {
    view: ALL_ROLES,
  },

  ops_center: {
    view: LEADERSHIP,   // ops center is admin/CEO/CTO only
  },
};

/**
 * Check if a role has permission to perform an action on a module.
 * custom_permissions overrides role-based permissions if defined.
 */
export function can(role, module, action, custom_permissions = null) {
  if (custom_permissions && custom_permissions[module] && typeof custom_permissions[module][action] === 'boolean') {
    return custom_permissions[module][action];
  }
  if (!role || !module || !action) return false;
  const normalizedRole = role.toLowerCase().replace(/ /g, '_');
  const allowedRoles = PERMISSIONS?.[module]?.[action];
  if (!allowedRoles) return false;
  return allowedRoles.includes(normalizedRole);
}

/**
 * Get all permissions for a given role, applying custom overrides if present.
 */
export function getPermissionsForRole(role, custom_permissions = null) {
  if (!role) return {};
  const result = {};
  for (const [module, actions] of Object.entries(PERMISSIONS)) {
    result[module] = {};
    for (const [action, allowedRoles] of Object.entries(actions)) {
      if (custom_permissions && custom_permissions[module] && typeof custom_permissions[module][action] === 'boolean') {
        result[module][action] = custom_permissions[module][action];
      } else {
        result[module][action] = allowedRoles.includes(role);
      }
    }
  }
  return result;
}

export default PERMISSIONS;

/** Returns true for the master admin account regardless of role. */
export function isMasterAdmin(email) {
  const masterEmail = (
    process.env.MASTER_ADMIN_EMAIL ||
    process.env.NEXT_PUBLIC_MASTER_ADMIN_EMAIL ||
    'manisreethaar@gmail.com'
  ).toLowerCase();
  return String(email || '').toLowerCase() === masterEmail;
}

/**
 * Check if fromRole can assign a task to toRole based on OxyOS hierarchy.
 * Rule: Master Admin can assign to everyone. CEO/CTO can assign to everyone.
 * Others can assign to self or below. CEO/CTO are protected from lower-role assignments.
 */
export function canAssignTo(fromRole, toRole, fromEmail = '') {
  if (isMasterAdmin(fromEmail)) return true;
  if (!fromRole || !toRole) return false;
  
  const weights = {
    'ceo': 100,
    'cto': 90,
    'admin': 80,
    'research_fellow': 70,
    'scientist': 60,
    'research_intern': 45,
    'intern': 40
  };

  const fromWeight = weights[fromRole] || 0;
  const toWeight = weights[toRole] || 0;

  if (fromRole === 'ceo' || fromRole === 'cto') return true;
  if (toRole === 'ceo' || toRole === 'cto') return false;
  return fromWeight >= toWeight;
}
