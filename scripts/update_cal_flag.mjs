...
async function run() {
  ...
  for (const item of dbEquipment) {
    ...
    if (isMatch) {
      console.log('Setting requires_calibration to false for: ' + item.name);
      await supabase.from('equipment').update({ requires_calibration: false }).eq('id', item.id);
      
      // Also delete any existing records from equipment_calibrations
  ...
  console.log('Successfully removed calibration tracking for ' + updatedCount + ' items.');
}
